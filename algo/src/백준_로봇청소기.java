...
public class 백준_로봇청소기 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        int RobbotN = Integer.parseInt(st.nextToken()); //현재 Y좌표
        int RobbotM = Integer.parseInt(st.nextToken()); //현재 X좌표
...
//            if(aim == 2) nextaim = 1;
//            if(aim == 3) nextaim = 2;
            int ddx = RobbotM + dx[nextaim]; //바라보고 있는 방향으로 한칸 이동
            int ddy = RobbotN + dy[nextaim];
            ...
            if(ddx>=0 && ddx <M && ddy >= 0 && ddy < N){
                if(map[ddy][ddx]==0){ //청소안한곳이면이동가능
                    count++;
                    move(ddy, ddx, nextaim);
                    return;
                }
            }
        }
        //위에서 이동 조건에 안맞는곳. 즉 다 막혀있어서 이동이 안되는 경우는 후진해야함.
        int nnext = (aim+2)%4; //반대 방향
        int nnx = RobbotM + dx[nnext];
        int nny = RobbotN + dy[nnext];
